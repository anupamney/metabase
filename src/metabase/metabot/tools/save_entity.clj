...
(defn- save-to-document!
  ...
  (let [document-id (:document_id destination)]
    ...
    (query-perms/check-run-permissions-for-query dataset_query)
    (let [document (api/write-check (documents/get-document document-id))
          ...
          card     (queries/create-card!
                    ...
                    {:id api/*current-user-id*})]
      (documents/add-card-to-document! document (:id card) (:position destination))
      {:card          card
       :location      {:type "document" :id document-id}
       :location-name (:name document)
