...
(defn persistable-data-part?
  ...
  duplicating the blob in every message would bloat storage. `entity_saved` parts are
  skipped too — they annotate a live inline chart with its saved location and are only
  meaningful within the session that created the chart. Non-data parts are always
